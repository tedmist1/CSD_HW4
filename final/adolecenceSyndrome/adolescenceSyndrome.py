...
class ObjectiveCaptureAgent(CaptureAgent):
    ...
    def chooseAction(self, gameState):
        ...
        if self.previousQ and self.previousReward:
            self.updateWeights(self.previousReward, self.previousQ, maxValue, self.getFeatures(gameState, bestActions[0]))
        self.previousQ = maxValue
        self.previousReward = self.reward# needtoDefine
...
class OffensiveObjectiveAgent(ObjectiveCaptureAgent):
    ...
    def getFeatures(self, gameState, action):
        ...
        if len(defenders) > 0:
            ...
            # Reward for if in danger
            self.reward = -5 * min(dists)
        ...
        # trying to give reward here? is there a way to know how many pellets are held
        if not inDanger:
            self.reward = gameState.getScore()
